...
router.get("/", function (req, res, next) {
  ...
  connection.query("SELECT * FROM link_fashion ORDER BY id ASC", function (err, rows) {
    if (err) {
      req.flash("error", err);
      res.render("link_fashion/index", { data: "" });
    } else {
      res.render("link_fashion/index", { data: rows });
    }
  });
});
...
router.get("/create", function (req, res, next) {
  res.render("link_fashion/create", {
    gambar_produk: "",
    nama_produk: "",
    link: "",
    ecommerce: "",
...
router.post("/store", upload, function (req, res, next) {
  ...
  connection.query("INSERT INTO link_fashion SET ?", formData, function (err, result) {
    if (err) {
      ...
      return res.redirect("/link_fashion");
    }
  });
});
...
router.get("/edit/(:id)", function (req, res, next) {
  ...
  connection.query(
    "SELECT * FROM link_fashion WHERE id = " + id,
    function (err, rows, fields) {
      ...
      if (rows.length <= 0) {
        ...
        res.redirect("/link_fashion");
      }
      // if book found
      else {
        // render to edit.ejs
        res.render("link_fashion/edit", {
          id: rows[0].id,
...
router.post('/update/:id', function(req, res, next) {
    upload(req, res, function(err) {
        if (err) {
            ...
            return res.redirect('/link_fashion/edit/' + req.params.id);
        }
        ...
        if (errors) {
            return res.render('link_fashion/edit', {
                id: id,
...
router.get('/delete/(:id)', function(req, res, next) {
    ...
    connection.query('DELETE FROM link_fashion WHERE id = ' + id, function(err, result) {
        //if(err) throw err
        if (err) {
            // set flash message
            req.flash('error', err)
